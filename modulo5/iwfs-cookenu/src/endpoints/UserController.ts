...
class UserController {
  ...
  follow = async (req: Request, res: Response) => {
    try {
      ...
      res.status(200).send({message: `Usuário ${user.getName()} está seguindo ${userExist.getName()} com sucesso!`});
    } catch (error: any) {
      res
      .status(error.statusCode || 500)
      .send({ message: error.message || error.sqlMessage })
  ...
  unfollow = async (req: Request, res: Response) => {
    try {
      ...
      res.status(200).send({message: `Usuário ${user.getName()} deixou de seguir ${userExist.getName()} com sucesso!`});
    } catch (error: any) {
      res
      .status(error.statusCode || 500)
      .send({ message: error.message || error.sqlMessage })
  ...
  getRecipesFeed = async (req: Request, res: Response) => {
    try {
        ...
        const recipes = await userDB.selectRecipesUserFeed(user.getId())
